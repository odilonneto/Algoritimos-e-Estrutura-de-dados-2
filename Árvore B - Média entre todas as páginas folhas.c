...
void conta_paginas(NodeB *tree, int *contador, int *soma){
    if(tree!=NULL){
        if(tree->eh_no_folha){
            NodeB *aux=tree;
            for(int i=0; i<aux->nro_chaves; i++){
                *soma+=aux->chaves[i];
                *contador+=1;
            }
        }
            ...
            for(int i=0; i<=tree->nro_chaves; i++)
                conta_paginas(tree->filhos[i], contador, soma);
    }
}

int main(){
    NodeB *tree=criar();
    int n, x, contador=0, soma=0;
    scanf("%d", &n);
    while(n>0){
        scanf("%d", &x);
        tree=inserir(tree, x);
    ...
    conta_paginas(tree, &contador, &soma);
